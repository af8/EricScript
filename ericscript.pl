...
if ($recalc != 0) {
	my $cmd = "R --slave --args $ericscriptfolder,$refid,$dbfolder < $ericscriptfolder/lib/R/CheckDB.R";
	system($cmd) == 0 or die("No (or incomplete) reference database found: $cmd");
	my $flagdb;
...
###############################################
### EricScript main processing wrapper bloc ###
###############################################
